...
You should see the slider popout of a gear icon in the upper left of your Plots pane in RStudio.
...
ggplot(aes(eruptions)) + 
geom_histogram(aes(y = ..density..),  bins = 20) +
geom_density(color='red', size=1, adjust=b) +
xlab('Duration (minutes)') +
ylab('Frequency density') +
ggtitle('Geyser Eruption Duration')

}, b = slider(min = 5, max = 50, label = 'bandwidth adjustment', step = 5))
```
...
all_values <- function(x) {
  if(is.null(x)) return(NULL)
  row <- cars.task[cars.task$id == x$id, ]
  paste0(names(row), ": ", format(row), collapse = "<br/>")
}
